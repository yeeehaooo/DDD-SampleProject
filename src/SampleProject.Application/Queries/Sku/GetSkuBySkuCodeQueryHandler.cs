...
public class GetSkuBySkuCodeQueryHandler : IRequestHandler<GetSkuBySkuCodeQuery, SkuDto?>
{
    ...
    public GetSkuBySkuCodeQueryHandler(
    ...
    public async Task<SkuDto?> HandleAsync(
        GetSkuBySkuCodeQuery request,
        CancellationToken cancellationToken = default)
    {
        var sku = await _skuRepository.GetBySkuCodeAsync(request.SkuCode, cancellationToken);
        if (sku == null)
            return null;
